...
import { EnterWorkflowNodeImpl } from '../enter_workflow_node_impl';

const mockParseDuration = parseDuration as jest.MockedFunction<typeof parseDuration>;

describe('EnterWorkflowNodeImpl', () => {
  let node: EnterWorkflowNode;
  ...
  let impl: EnterWorkflowNodeImpl;
  ...
  beforeEach(() => {
    ...
    impl = new EnterWorkflowNodeImpl(
      node,
      wfExecutionRuntimeManagerMock,
      stepExecutionRuntimeFactoryMock
    );
  ...
  describe('monitor method', () => {
    ...
    it('should be a no-op when timeout is not configured', () => {
      const nodeWithoutTimeout: EnterWorkflowNode = {
        ...node,
        timeout: undefined,
      };
      const implWithoutTimeout = new EnterWorkflowNodeImpl(
        nodeWithoutTimeout,
        wfExecutionRuntimeManagerMock,
        stepExecutionRuntimeFactoryMock
      );

      implWithoutTimeout.monitor(monitoredStepExecutionRuntimeMock);
    ...
    it('should handle different timeout formats', async () => {
      ...
      mockParseDuration.mockReturnValue(120000);
      wfExecutionRuntimeManagerMock.getWorkflowExecution = jest.fn().mockReturnValue({
        startedAt: new Date(startTime).toISOString(),
      });
    ...
    it('should parse timeout correctly for various duration formats', async () => {
      const recentStartTime = new Date().getTime() - 1000;
    ...
    it('should use correct time calculations', async () => {
      const startTime = mockDateNow.getTime() - 90000;
      mockParseDuration.mockReturnValue(60000);
    ...
    it('should timeout when duration is greater than timeout by 1ms', async () => {
      const startTime = new Date().getTime() - 60001;
      mockParseDuration.mockReturnValue(60000);
